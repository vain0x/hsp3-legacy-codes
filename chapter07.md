...
になります。
ちなみに、もし他にも ``long_何々`` というファイルがある場合は、タブキーを繰り返し押すことで選択できます。
...
と書けば、リポジトリ内のすべてのファイルが git add されます。
...
これは便利ですね。
ウィザードが登録するエイリアスでは、 ``cia = git comit -a -v`` になっています。(-v は次節で説明します。)
...
そういうエイリアス (``cid = !git diff && git adf .&& git commit`` みたいな) を作ってもいいですが、git の機能を利用してうまくやる方法があります。
それが git commit -v です。
...
※ちなみに、一般的に commit は ci と略されることが多いみたいです。

## add の機能？
これまで git add は git commit と常にペアで扱ってきました。
すなわち、add 単体の機能について完全に無視してきたのです。
...
* git commit は、「インデックス」に追加されている変更を、1個のコミットオブジェクトにする。(そしてHEADを移動する。)
  * この後、インデックスか空になる。

作業ディレクトリ → インデックス → コミットオブジェクト、という流れがあるのが分かります。
...
## 次回予告
さて、やっと第1部が終わりましたね。
何度もいいますが、これでみなさんはいっぱしの git 使いになりましたよ！

ただし、ここまでの内容は、どのバージョン管理ツールでもできることです。
第2部では、git の本領である **ブランチ**(branch) について解説したいと思います。

では、また [次回](chapter08.md) 。
