// JSON 読み込みクラス (using obsidian)

#ifndef IG_MODULECLASS_OBSIDIAN_JSON_LEXER_AS
#define IG_MODULECLASS_OBSIDIAN_JSON_LEXER_AS
...
//                  定義部 : MCObsJsonLexer
//##############################################################################
#module MCObsJsonLexer mSrc, mlen, tktype, tkstr, tklen, idx, cntToken, tktypelist, tkstrlist
...
//        構築・解体
//##########################################################
#define global obsjsonLexer_new(%1,%2) newmod %1, MCObsJsonLexer@, %2
#define global obsjsonLexer_delete(%1) delmod %1
...
// 字句解析
//------------------------------------------------
#modfunc obsjsonLexer_Lex array tklist
	dim tktypelist
	sdim tkstrlist
	...
	repeat
		...
		// 次のトークンを取得
		obsjsonLexer_Lex_GetNextToken thismod, tkstr, idx
		tktype = stat
		tklen  = strlen(tkstr)
		idx   += tklen
	...
	tklist = vector( vector_from_array( tktypelist ), vector_from_array( tkstrlist ) )
	return cntToken
...
// 文字列か文字定数を切り出す
//------------------------------------------------
#defcfunc CutStr_or_Char@MCObsJsonLexer var sSrc, int iOffset, int p3
...
// 空白を切り出して返す
//------------------------------------------------
#defcfunc CutBlank@MCObsJsonLexer var sSrc, int iOffset
...
#defcfunc CutIdent@MCObsJsonLexer var sSrc, int iOffset
	FTM_CutToken ( IsIdent(c) || IsSJIS1st(c) ), IsSJIS1st(c)
	
//------------------------------------------------
// 10進数を切り出して返す
//------------------------------------------------
#defcfunc CutNum_Dgt@MCObsJsonLexer var sSrc, int iOffset
...
// 次のトークンを取得する
//------------------------------------------------
#modfunc obsjsonLexer_Lex_GetNextToken@MCObsJsonLexer var result, int idx_,  local restype
	idx = idx_
	c = peek(mSrc, idx)
