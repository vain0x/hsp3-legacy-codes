...
//-----------------------------------------------------------------------------
int GetDropFileList(HDROP hDrop,char* filelist,int bufsize)
{
	char buf[_MAX_PATH], *p;
	int i;
	p = filelist;
	if (!p) return -1;
	for (i=0; i<(int)DragQueryFile(hDrop,-1,NULL,0); i++) {
		DragQueryFile(hDrop,i,buf,ARRAYSIZE(buf)-1);
		if (p+lstrlen(buf)-filelist>bufsize) break;
		lstrcpy(p,buf);
		p += lstrlen(buf) + 1;
	}
	*p = '\0';
...
/*----------------------
	IDropTarget
----------------------*/
HRESULT STDMETHODCALLTYPE OLE_IDropTarget_QueryInterface(LPDROPTARGET pThis, REFIID riid, LPVOID *ppvObject);
ULONG STDMETHODCALLTYPE OLE_IDropTarget_AddRef(LPDROPTARGET pThis);
ULONG STDMETHODCALLTYPE OLE_IDropTarget_Release(LPDROPTARGET pThis);
HRESULT STDMETHODCALLTYPE OLE_IDropTarget_DragEnter(LPDROPTARGET pThis, LPDATAOBJECT pdo, DWORD grfKeyState, POINTL pt, LPDWORD pdwEffect);
HRESULT STDMETHODCALLTYPE OLE_IDropTarget_DragOver(LPDROPTARGET pThis, DWORD grfKeyState, POINTL pt, LPDWORD pdwEffect);
HRESULT STDMETHODCALLTYPE OLE_IDropTarget_DragLeave(LPDROPTARGET pThis);
HRESULT STDMETHODCALLTYPE OLE_IDropTarget_Drop(LPDROPTARGET pThis, LPDATAOBJECT pdo, DWORD grfKeyState, POINTL pt, LPDWORD pdwEffect);
HRESULT APIPRIVATE OLE_IDropTarget_Internal_SendMessage(LPDROPTARGET pThis,UINT uNotify,LPDATAOBJECT pdo,DWORD grfKeyState, POINTL *ppt, LPDWORD pdwEffect);
...
static HRESULT APIPRIVATE DropTarget_GetData(LPDATAOBJECT pdo,UINT cfFormat,LPSTGMEDIUM psm)
{
	...
	fmt.cfFormat = (CLIPFORMAT)cfFormat;
	fmt.ptd = NULL;
