// HSP定数

// from hsp3struct.h

#ifndef IG_HSP_CONSTANT_NUMBERS_AS
#define IG_HSP_CONSTANT_NUMBERS_AS

// type値
#define global TYPE_MARK   0
#define global TYPE_VAR    1
#define global TYPE_STRING 2
#define global TYPE_DNUM   3
#define global TYPE_INUM   4
#define global TYPE_STRUCT 5
#define global TYPE_XLABEL 6
#define global TYPE_LABEL  7
#define global TYPE_INTCMD 8
#define global TYPE_EXTCMD 9
#define global TYPE_EXTSYSVAR 10
#define global TYPE_CMPCMD  11
#define global TYPE_MODCMD  12
#define global TYPE_INTFUNC 13
#define global TYPE_SYSVAR  14
#define global TYPE_PROGCMD 15
#define global TYPE_DLLFUNC 16
#define global TYPE_DLLCTRL 17
#define global TYPE_USERDEF 18

#define global TYPE_ERROR     (-1)
#define global TYPE_CALCERROR (-2)

// コードブロック系
#define global EXFLG_0 0x1000
#define global EXFLG_1 0x2000
#define global EXFLG_2 0x4000
#define global EXFLG_3 0x8000
#define global CSTYPE  0x0FFF

// code_get() 戻り値
#define PARAM_OK       0
#define PARAM_SPLIT    (-1)
#define PARAM_END      (-2)
#define PARAM_DEFAULT  (-3)
#define PARAM_ENDSPLIT (-4)

// 最大値
#define HSP3_FUNC_MAX  TYPE_USERDEF
#define HSP3_TYPE_USER TYPE_USERDEF

/*
struct HSPHED
{
	char	h1;					// magic code1
	char	h2;					// magic code2
	char	h3;					// magic code3
	char	h4;					// magic code4
	int		version;			// version number info
	int		max_val;			// max count of VAL Object
	int		allsize;			// total file size
	
	int		pt_cs;				// ptr to Code Segment
	int		max_cs;				// size of CS
	int		pt_ds;				// ptr to Data Segment
	int		max_ds;				// size of DS
	
	int		pt_ot;				// ptr to Object Temp
	int		max_ot;				// size of OT
	int		pt_dinfo;			// ptr to Debug Info
	int		max_dinfo;			// size of DI

	int		pt_linfo;			// ptr to LibInfo(2.3)
	int		max_linfo;			// size of LibInfo(2.3)
	int		pt_finfo;			// ptr to FuncInfo(2.3)
	int		max_finfo;			// size of FuncInfo(2.3)

	int		pt_minfo;			// ptr to ModInfo(2.5)
	int		max_minfo;			// size of ModInfo(2.5)
	int		pt_finfo2;			// ptr to FuncInfo2(2.5)
	int		max_finfo2;			// size of FuncInfo2(2.5)

	int		pt_hpidat;			// ptr to HPIDAT(3.0)
	short		max_hpi;			// size of HPIDAT(3.0)
	short		max_varhpi;			// Num of Vartype Plugins(3.0)
	int		bootoption;			// bootup options
	int		runtime;			// ptr to runtime name
};
//*/

//#define global HSPHED_BOOTOPT_WINHIDE 2			// 起動時ウインドウ非表示
//#define global HSPHED_BOOTOPT_DIRSAVE 4			// 起動時カレントディレクトリ変更なし
#define global HSPHED_BOOTOPT_DEBUGWIN 1			// 起動時デバッグウインドゥ表示
//#define global HSPHED_BOOTOPT_SAVER 0x100			// スクリーンセーバー
#define global HSPHED_BOOTOPT_RUNTIME 0x1000		// 動的ランタイムを有効にする

// HPIDATフラグ
#define HPIDAT_FLAG_TYPEFUNC 0
#define HPIDAT_FLAG_VARFUNC 1
#define HPIDAT_FLAG_DLLFUNC 2

/*
struct HPIDAT {
	short	flag;				// flag info
	short	option;
	int		libname;			// lib name index (DS)
	int		funcname;			// function name index (DS)
	void	*libptr;			// lib handle
};
//*/

// LIBDATフラグ
#define LIBDAT_FLAG_NONE 0
#define LIBDAT_FLAG_DLL 1
#define LIBDAT_FLAG_DLLINIT 2
#define LIBDAT_FLAG_MODULE 3
#define LIBDAT_FLAG_COMOBJ 4

/*
struct LIBDAT {
	int		flag;				// initalize flag
	int		nameidx;			// function name index (DS)
								// Interface IID ( Com Object )
	void	*hlib;				// Lib handle
	int		clsid;				// CLSID (DS) ( Com Object )
};
//*/

// マルチパラメータタイプ
#define global MPTYPE_NONE   0				// -- (なし)
#define global MPTYPE_VAR    1				// var
#define global MPTYPE_STRING 2				// str
#define global MPTYPE_DNUM   3				// double
#define global MPTYPE_INUM   4				// int
#define global MPTYPE_STRUCT 5				// -- (struct)
#define global MPTYPE_LABEL  7				// label

#define global MPTYPE_LOCALVAR    0xFFFF	// local
#define global MPTYPE_ARRAYVAR    0xFFFE	// array
#define global MPTYPE_SINGLEVAR   0xFFFD	// var
#define global MPTYPE_FLOAT       0xFFFC	// float
#define global MPTYPE_STRUCTTAG   0xFFFB	// 
#define global MPTYPE_LOCALSTRING 0xFFFA	// str (内部)
#define global MPTYPE_MODULEVAR   0xFFF9	// modvar
#define global MPTYPE_PPVAL       0xFFF8	// pval
#define global MPTYPE_PBMSCR      0xFFF7	// bmscr
#define global MPTYPE_PVARPTR     0xFFF6	// var (#func)
#define global MPTYPE_IMODULEVAR  0xFFF5	// modinit

#define global MPTYPE_IOBJECTVAR  0xFFF4	// comobj
#define global MPTYPE_LOCALWSTR   0xFFF3	// 
#define global MPTYPE_FLEXSPTR    0xFFF2	// 
#define global MPTYPE_FLEXWPTR    0xFFF1	// 
#define global MPTYPE_PTR_REFSTR  0xFFF0	// prefstr
#define global MPTYPE_PTR_EXINFO  0xFFEF	// pexinfo
#define global MPTYPE_PTR_DPMINFO 0xFFEE	// pdpminfo
#define global MPTYPE_NULLPTR     0xFFED	// nullptr
#define global MPTYPE_TMODULEVAR  0xFFEC	// modterm

//#define global MPTYPE_PTR_HWND  -14
//#define global MPTYPE_PTR_HDC   -15
//#define global MPTYPE_PTR_HINST -16

// STRUCTDAT::subid, STRUCTPRM::subid に入る値
#define global STRUCTPRM_SUBID_STACK      (-1)		// [PRM] prmstack 要素
#define global STRUCTPRM_SUBID_STID       (-2)		// [PRM] (funcと関係がありそうだが、不明)
#define global STRUCTPRM_SUBID_DLL        (-3)		// [DAT] Dll関数
#define global STRUCTPRM_SUBID_DLLINIT    (-4)		// [DAT] ↑の実行時初期化が完了済みである状態
#define global STRUCTPRM_SUBID_OLDDLL     (-5)		// [DAT] Dll関数 (「#func ident ".." 数値」という古い形式)
#define global STRUCTPRM_SUBID_OLDDLLINIT (-6)		// [DAT] ↑の実行時初期化が完了済みである状態
#define global STRUCTPRM_SUBID_COMOBJ     (-7)		// [DAT] #comfunc

#define global STRUCTCODE_THISMOD (-1)				// thismod を示す code 値

#define global TYPE_OFFSET_COMOBJ 0x1000			// STRUCTDAT::subid へのバイアス

/*
struct STRUCTPRM {
	short	mptype;				// Parameter type
	short	subid;				// struct index
	int		offset;				// offset from top
};
//*/

// DLL function flags
#define global STRUCTDAT_OT_NONE      0x0000
#define global STRUCTDAT_OT_CLEANUP   0x0001
#define global STRUCTDAT_OT_STATEMENT 0x0002
#define global STRUCTDAT_OT_FUNCTION  0x0004

// Module function flags
#define global STRUCTDAT_INDEX_FUNC   (-1)
#define global STRUCTDAT_INDEX_CFUNC  (-2)
#define global STRUCTDAT_INDEX_STRUCT (-3)

#define global STRUCTDAT_FUNCFLAG_CLEANUP 0x10000

/*
// function, module specific data
struct STRUCTDAT {
	short	index;				// base LIBDAT index
	short	subid;				// struct index
	int		prmindex;			// STRUCTPRM index(MINFO)
	int		prmmax;				// number of STRUCTPRM
	int		nameidx;			// name index (DS)
	int		size;				// struct size (stack)
	int		otindex;			// OT index(Module) / cleanup flag(Dll)
	union {
	void	*proc;				// proc address
	int		funcflag;			// function flags(Module)
	};
};
//*/

/*
//	Var Data for Multi Parameter
struct MPVarData {
	PVal	*pval;
	APTR	aptr;
};
//*/

/*
//	Var Data for Module Function
typedef struct MPModVarData {
	short subid;
	short magic;
	PVal	*pval;
	APTR	aptr;
} MPModVarData;
//*/
#define global MODVAR_MAGICCODE 0x55AA

// IRQフラグ
#define global IRQ_FLAG_NONE    0
#define global IRQ_FLAG_DISABLE 1
#define global IRQ_FLAG_ENABLE  2
#define global IRQ_OPT_GOTO     0
#define global IRQ_OPT_GOSUB    1
#define global IRQ_OPT_CALLBACK 2

/*
//	Stack info for DLL Parameter
struct MPStack {
	char *prmbuf;
	char **prmstk;
	int curstk;
	void *vptr;
};
//*/

/*
struct IRQDAT {
	short	flag;								// flag
	short	opt;								// option value
	int		custom;								// custom message value
	int		custom2;							// custom message value2
	int		iparam;								// iparam option
	unsigned short *ptr;						// jump ptr
	void	(*callback)(struct IRQDAT *,int,int);		// IRQ callback function
};
//*/

//typedef struct HSPCTX HSPCTX;

/*
//	Plugin info data (3.1 or later)
typedef struct HSPEXINFO
{
	//		HSP internal info data (2.6)
	//
	short ver;		// Version Code
	short min;		// Minor Version
	//
	int *er;		// Not Use
	char *pstr;		// String Buffer (master)
	char *stmp;		// String Buffer (sub)
	PVal **mpval;		// Master PVAL
	//
	int *actscr;		// Active Window ID
	int *nptype;		// Next Parameter Type
	int *npval;			// Next Parameter Value
	int *strsize;		// StrSize Buffer
	char *refstr;		// RefStr Buffer
	//
	void *(*HspFunc_prm_getv)( void );
	int (*HspFunc_prm_geti)( void );
	int (*HspFunc_prm_getdi)( const int defval );
	char *(*HspFunc_prm_gets)( void );
	char *(*HspFunc_prm_getds)( const char *defstr );
	int (*HspFunc_val_realloc)( PVal *pv, int size, int mode );
	int (*HspFunc_fread)( char *fname, void *readmem, int rlen, int seekofs );
	int (*HspFunc_fsize)( char *fname );
	void *(*HspFunc_getbmscr)( int wid );
	int (*HspFunc_getobj)( int wid, int id, void *inf );
	int (*HspFunc_setobj)( int wid, int id, const void *inf );

	//		HSP internal info data (3.0)
	//
	int *npexflg;	// Next Parameter ExFlg
	HSPCTX *hspctx;	// HSP context ptr

	//		Enhanced data (3.0)
	//
	int (*HspFunc_addobj)( int wid );
	void (*HspFunc_puterror)( HSPERROR error );
	HspVarProc *(*HspFunc_getproc)( int type );
	HspVarProc *(*HspFunc_seekproc)( const char *name );

	void (*HspFunc_prm_next)( void );
	int (*HspFunc_prm_get)( void );
	double (*HspFunc_prm_getd)( void );
	double (*HspFunc_prm_getdd)( double defval );
	unsigned short *(*HspFunc_prm_getlb)( void );
	PVal *(*HspFunc_prm_getpval)( void );
	APTR (*HspFunc_prm_getva)( PVal **pval );
	void (*HspFunc_prm_setva)( PVal *pval, APTR aptr, int type, const void *ptr );
	char *(*HspFunc_malloc)( int size );
	void (*HspFunc_free)( void *ptr );
	char *(*HspFunc_expand)( char *ptr, int size );
	IRQDAT *(*HspFunc_addirq)( void );
	int (*HspFunc_hspevent)( int event, int prm1, int prm2, void *prm3 );
	void (*HspFunc_registvar)( int flag, HSPVAR_COREFUNC func );
	void (*HspFunc_setpc)( const unsigned short *pc );
	void (*HspFunc_call)( const unsigned short *pc );
	void (*HspFunc_mref)( PVal *pval, int prm );

	void (*HspFunc_dim)( PVal *pval, int flag, int len0, int len1, int len2, int len3, int len4 );
	void (*HspFunc_redim)( PVal *pval, int lenid, int len );
	void (*HspFunc_array)( PVal *pval, int offset );

	//		Enhanced data (3.1)
	//
	char *(*HspFunc_varname)( int id );
	int (*HspFunc_seekvar)( const char *name );

};
//*/

#define global HSP3_REPEAT_MAX 32
#define global ETRLOOP ((int)0x80000000)

/*
struct LOOPDAT {
	int		time;				// loop times left
	int		cnt;				// count
	int		step;				// count add value
	unsigned short *pt;			// loop start ptr
};
//*/

// 実行モード
#enum global RUNMODE_RUN = 0
#enum global RUNMODE_WAIT
#enum global RUNMODE_AWAIT
#enum global RUNMODE_STOP
#enum global RUNMODE_END
#enum global RUNMODE_ERROR
#enum global RUNMODE_RETURN
#enum global RUNMODE_INTJUMP
#enum global RUNMODE_ASSERT
#enum global RUNMODE_LOGMES
#enum global RUNMODE_EXITRUN
#enum global RUNMODE_MAX

/*
struct HSPCTX
{
	HSPHED *hsphed;						// HSP object file header
	unsigned short *mcs;				// current code segment ptr
	unsigned short *mem_mcs;			// code segment ptr
	char *mem_mds;						// data segment ptr
	unsigned char *mem_di;				// Debug info ptr
	int *mem_ot;						// object temp segment ptr

	IRQDAT *mem_irq;					// IRQ data ptr
	int irqmax;							// IRQ data count
	int iparam;							// IRQ Info data1
	int wparam;							// IRQ Info data2
	int lparam;							// IRQ Info data3

	PVal *mem_var;						// var storage index
	HSPEXINFO30 exinfo;					// HSP function data(3.0)
	int runmode;						// HSP execute mode
	int waitcount;						// counter for wait
	int waitbase;						// wait sleep base
	int waittick;						// next tick for await
	int lasttick;						// previous tick
	int sublev;							// subroutine level
	LOOPDAT mem_loop[HSP3_REPEAT_MAX];	// repeat loop info
	int looplev;						// repeat loop level
	HSPERROR err;						// error code
	int hspstat;						// HSP status
	int stat;							// sysvar 'stat'
	int strsize;						// sysvar 'strsize'
	char *refstr;						// RefStr Buffer
	char *fnbuffer;						// buffer for FILENAME
	void *instance;						// Instance Handle (windows)
	int intwnd_id;						// Window ID (interrupt)
	PVal *note_pval;					// MemNote pval
	APTR note_aptr;						// MemNote aptr
	PVal *notep_pval;					// MemNote pval (previous)
	APTR notep_aptr;					// MemNote aptr (previous)
	char *stmp;							// String temporary buffer

	void *prmstack;						// Current parameter stack area
	LIBDAT *mem_linfo;					// Library info
	STRUCTPRM *mem_minfo;				// Parameter info
	STRUCTDAT *mem_finfo;				// Function/Struct info
	int retval_level;					// subroutine level (return code)
	int endcode;						// End result code
	void (*msgfunc) (HSPCTX *);			// Message Callback Proc.
	void *wnd_parent;					// Parent Window Handle
	double refdval;						// sysvar 'refdval'
	char *cmdline;						// Command Line Parameters

	HSPEXINFO *exinfo2;					// HSP function data(3.1)
};
//*/

#define global HSPCTX_REFSTR_MAX  4096
#define global HSPCTX_CMDLINE_MAX 1024

#define global HSPSTAT_NORMAL 0
#define global HSPSTAT_DEBUG  1
#define global HSPSTAT_SSAVER 2

#define global TYPE_EX_SUBROUTINE 0x100		// gosub用のスタックタイプ
#define global TYPE_EX_CUSTOMFUNC 0x101		// deffunc呼び出し用のスタックタイプ
#define global TYPE_EX_ENDOFPARAM 0x200		// パラメーター終端(HSPtoC)
#define global TYPE_EX_ARRAY_VARS 0x201		// 配列要素付き変数用スタックタイプ(HSPtoC)

/*
struct HSPROUTINE
{
	//	Subroutine Context
	int stacklev;						// サブルーチン開始時のスタックレベル
	int looplev;						// サブルーチン開始時の repeat ループレベル
	unsigned short *mcsret;				// 呼び出し元PCポインタ(復帰用)
	STRUCTDAT *param;					// 引数パラメーターリスト
	void *oldtack;						// 以前のスタックアドレス
};
//*/

// コールバックのオプション
#define HSPEVENT_ENABLE_COMMAND 0x0001	// １ステップ実行時
#define HSPEVENT_ENABLE_HSPIRQ  0x0002	// HSP内での割り込み発生時
#define HSPEVENT_ENABLE_GETKEY  0x0004	// キーチェック時
#define HSPEVENT_ENABLE_FILE    0x0008	// ファイル入出力時
#define HSPEVENT_ENABLE_MEDIA   0x0010	// メディア入出力時
#define HSPEVENT_ENABLE_PICLOAD 0x0020	// picload命令実行時

/*
struct HSP3TYPEINFO {
	//	型ごとの情報
	//	(*の項目は、親アプリケーションで設定されます)
	
	short type;							// *型タイプ値
	short option;						// *オプション情報
	HSPCTX *hspctx;						// *HSP Context構造体へのポインタ
	HSPEXINFO *hspexinfo;				// *HSPEXINFO構造体へのポインタ
	
	//	ファンクション情報
	int (* cmdfunc) (int);				// コマンド受け取りファンクション
	void *(* reffunc) (int *,int);		// 参照受け取りファンクション
	int (* termfunc) (int);				// 終了受け取りファンクション

	// イベントコールバックファンクション
	int (* msgfunc) (int,int,int);				// Windowメッセージコールバック
	int (* eventfunc) (int,int,int,void *);		// HSPイベントコールバック
};
//*/

// HSP割り込みID
#enum global HSPIRQ_ONEXIT = 0
#enum global HSPIRQ_ONERROR
#enum global HSPIRQ_ONKEY
#enum global HSPIRQ_ONCLICK
#enum global HSPIRQ_USERDEF
#enum global HSPIRQ_MAX

// HSPイベントID
#enum global HSPEVENT_NONE = 0
#enum global HSPEVENT_COMMAND
#enum global HSPEVENT_HSPIRQ
#enum global HSPEVENT_GETKEY
#enum global HSPEVENT_STICK
#enum global HSPEVENT_FNAME
#enum global HSPEVENT_FREAD
#enum global HSPEVENT_FWRITE
#enum global HSPEVENT_FEXIST
#enum global HSPEVENT_FDELETE
#enum global HSPEVENT_FMKDIR
#enum global HSPEVENT_FCHDIR
#enum global HSPEVENT_FCOPY
#enum global HSPEVENT_FDIRLIST1
#enum global HSPEVENT_FDIRLIST2
#enum global HSPEVENT_GETPICSIZE
#enum global HSPEVENT_PICLOAD
#enum global HSPEVENT_MAX

// sizeof(構造体)
#const global SizeOf_LIBDAT		16
#const global SizeOf_STRUCTDAT	28
#const global SizeOf_STRUCTPRM	8
#const global SizeOf_HPIDAT		16
#const global SizeOf_PVal		48

#endif
