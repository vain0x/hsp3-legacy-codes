...
#ifndef IG_MODULECLASS_STRSET_AS
#define IG_MODULECLASS_STRSET_AS
...
#define EXPAND_ALLOC_SIZE 0x00FF
...
#modcfunc    __StrSet_IsLastChar var p2, int lenOf_p2
...
//------------------------------------------------
#modcfunc _StrSet_SearchByNum int no, int iFrom,  local i, local n, local c, local c2
	i  = iFrom
	n  = 0
	c2 = peek(mChar)
	while ( n < no )
...
#modcfunc __StrSet_FindStr str p2, int iFrom, int lenOf_p2,  local stmp, local len
...
// すでに項目として存在するか
//------------------------------------------------
#modcfunc _StrSet_IsInserted str p2
...
// 最後の項目までのインデックス値を求める
//------------------------------------------------
#modcfunc _StrSet_IdxOfLast  local i
	// 最後を探索
...
// コンストラクタ
//------------------------------------------------
;#deffunc __init modinit MCStrSet@, str p2, str p3
...
// while の反復条件に使う
//------------------------------------------------
#modcfunc StrSet_iterCheck  local bool
...
// [i] 繰返子更新
//------------------------------------------------
#modcfunc StrSet_iterNext  var vIt, var iterData
	if ( iterData == false ) {
		dup vIt, mIt
		iterData = true
	}
...
#define global StrSet_cntItems StrSet_size
...
// 区切り文字を変換してから取得
//------------------------------------------------
#modcfunc StrSet_getall_byChar str newChar, local copy
...
// 最後に起きたエラー
//------------------------------------------------
#modcfunc StrSet_getLastErr
	return mLastErr
	
#ifdef _DEBUG
#modfunc ss_check
	logmes dbgstr(strlen(mString))
	logmes dbgstr(mStrLen        )
	return
#endif
